...
/** Nav body — used in desktop sidebar and mobile sheet. */
      ...
      <div className="flex h-14 shrink-0 items-center justify-between px-3.5">
        {collapsed ? (
          <Link
            to="/"
            onClick={onNavClick}
            className="grid h-7 w-7 place-items-center rounded-md border border-border bg-surface-2"
          >
          ...
          <Link to="/" onClick={onNavClick} className="min-w-0">
            <Logo />
          </Link>
        )}
        {showToggle && onToggle && (
          <button
            type="button"
            onClick={onToggle}
            className="grid h-7 w-7 cursor-pointer place-items-center rounded-md text-muted-foreground transition hover:bg-sidebar-accent hover:text-foreground"
          ...
          {nav.map((item) => {
            ...
            return (
              ...
              >
                ...
                <Icon
                  className={`h-4 w-4 shrink-0 ${active ? "text-foreground" : "text-muted-foreground group-hover:text-foreground"}`}
                />
                {!collapsed && (
                  <>
...
export function AppSidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  return (
    ...
    >
      <AppSidebarContent collapsed={collapsed} onToggle={onToggle} showToggle />
    </aside>
  );
}

function IconDash(p: { className?: string }) {
  return (
    <svg viewBox="0 0 16 16" className={p.className} fill="none" stroke="currentColor" strokeWidth="1.5">
...
function IconUpload(p: { className?: string }) {
  return (
    <svg viewBox="0 0 16 16" className={p.className} fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M8 10V3m0 0L5 6m3-3 3 3M3 11v1a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2v-1" />
    </svg>
  );
}
function IconReview(p: { className?: string }) {
  return (
    <svg viewBox="0 0 16 16" className={p.className} fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M2 4h12M2 8h12M2 12h7" />
      <path d="m11 12 1.2 1.2L15 10.5" />
    </svg>
  );
}
function IconAudit(p: { className?: string }) {
  return (
    <svg viewBox="0 0 16 16" className={p.className} fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M4 2h6l3 3v9a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V3a1 1 0 0 1 1-1Z" />
      <path d="M6 8h5M6 11h4" />
    </svg>
  );
}
function IconSource(p: { className?: string }) {
  return (
    <svg viewBox="0 0 16 16" className={p.className} fill="none" stroke="currentColor" strokeWidth="1.5">
...
function IconGear(p: { className?: string }) {
  return (
    <svg viewBox="0 0 16 16" className={p.className} fill="none" stroke="currentColor" strokeWidth="1.5">
